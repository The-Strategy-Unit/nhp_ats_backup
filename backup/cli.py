...
def _latest_snapshot() -> str:
    ...
    status_client = blob_service.get_blob_client(
    ...
    # Download the snapshot to a local file
...
def main() -> int:
    ...
    try:
        table_name = _get_env("PROD_TABLE_NAME")
    except EnvironmentError as e:
        print(f"Error: {e}")
        return 1

    # --- Backup prompt ---
    if _confirm(f"Would you like to back up your table [{table_name}] [y/N]?"):
        run_backup()
    ...
    # --- Restore prompt ---
    if _confirm(f"Would you like to restore your table [{table_name}] [y/N]?"):
        try:
            snapshot_path = _latest_snapshot()
        except Exception as e:
            print(f"Failed to fetch latest snapshot: {e}")
            return 1
        run_restore(snapshot_path)
